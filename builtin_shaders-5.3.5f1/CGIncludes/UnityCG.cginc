...
// Transforms direction from object to world space  把方向矢量从模型空间转换到世界空间
inline float3 UnityObjectToWorldDir( in float3 dir )
{
	return normalize(mul((float3x3)_Object2World, dir));
}

// Transforms direction from world to object space 把方向矢量从世界空间 转换到模型空间
inline float3 UnityWorldToObjectDir( in float3 dir ) 
{
	return normalize(mul((float3x3)_World2Object, dir));
}

// Transforms normal from object to world space  把法线从模型空间转换到世界空间  = 法线 * _Object2World 
// 去除最后一维，这时候矩阵是正交的  mul(m,v) = mul(v,tranpose(m));
...
inline float4 ComputeScreenPos (float4 pos) {
	//这边获取的值，用 tex2Dproj 采样时，会做除以z操作，转换到ndc空间(-1,1)，然后在转换到纹理空间(0,1)
	//f(x) = (x+1)*0.5    因为tex2Dproj自动执行除以Z.所以这边要先乘以Z
	//投影贴图映射
	//阴影贴图采用的是渲染到纹理的方式，这时候投影矩阵的y会被处理为-y
	//经过下面 计算，DX的值还是会变成
	//𝑢 = 0.5𝑥 + 0.5
	//𝑣 = −0.5𝑦 + 0.5

	float4 o = pos * 0.5f;
	#if defined(UNITY_HALF_TEXEL_OFFSET)//Directly Mapping Texels to Pixels //纹理映射到像素有0.5偏移 https://msdn.microsoft.com/en-us/library/bb219690(v=vs.85).aspx
...
//该函数是在使用了直接拷贝后台RT数据的时候用的，应该这个时候是直接拷贝渲染好的图的，这时候内部没有对y进行处理，所以DX下面需要自己翻转y= 1-y
